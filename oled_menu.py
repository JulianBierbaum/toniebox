...
class OLEDMenu:
    ...
    def __init__(self, encoder_clk=17, encoder_dt=20, confirm_pin=16):
        ...
        # Display buffer for smoother updates
        self.width = 128
        self.height = 64
    ...
    def _display_loop(self):
        ...
        last_update = 0
        min_update_interval = 0.03  # ~30fps cap

        while self.running:
            try:
                try:
                    update_type = self.update_queue.get(timeout=0.1)
    ...
    def _update_display_internal(self, update_type):
        ...
        if update_type == "main":
            ...
            self._draw_current_audio(self.current_audio)
        ...
        if steps != 0:
            with self.menu_lock:
                if steps > 0:
                    for _ in range(
                        min(steps, 3)
                    ):  # Limit step processing to prevent lag
